...
<section class="template-default template-platforms">
    <div class="container">
        ...
        <p class="title is-5">{{ sprintf(_('Platform: %s'), $platform->name) }}</p>

        @if ($errors->any())
            @include('forms.errors', ['class' => 'is-danger', 'text' => _('Errors found')])
        @endif
    
        <form action="{{ route('platforms.update', $platform) }}" method="post" enctype="multipart/form-data" id="form-edit">
            @csrf
            @method('PUT')
            ...
            <fieldset class="improved">
                ...
                <div class="field">
                    ...
                    <div class="control has-icons-left has-icons-right">
                        <input type="text" class="input" name="name_<?=$langCode?>" id="name_<?=$langCode?>" value="<?=old("name_{$langCode}", $platform->{'name_' . $langCode})?>" minlength="5" maxlength="200" required="required" placeholder="" autocomplete="off">
                ...
                <div class="field">
                    ...
                    <div class="control">
                        <textarea class="textarea" id="description_<?=$langCode?>" name="description_<?=$langCode?>" cols="30" rows="10"><?=old("description_{$langCode}", $platform->{'description_' . $langCode})?></textarea>
                    </div>
            ...
            <x-form-image-field name="logo" :help="$supportedImageFormats" :image="$image" :filename="$platform->logo"></x-form-image-field>
        </form>
    
        <form action="{{ route('platforms.delete', $platform) }}" method="post" id="form-delete">
            @csrf
            @method('DELETE')
        </form>
        ...
        <div class="buttons">
            ...
            <a href="{{ route('platforms.create') }}" class="button is-primary" form="form-edit">
