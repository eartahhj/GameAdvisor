...
@section('title') {{ _('Edit platform') }} @endsection
...
<section class="template-default template-platforms">
    <div class="container">
        ...
        <h2>{{ _('Edit game platform') }}</h2>
        <p>{{ _('Platform:') . ' ' . $platform->name }}</p>
        @if ($errors->any())
        ...
        <form action="{{ route('platforms.update', $platform)}}" method="post" enctype="multipart/form-data" id="form-edit">
            ...
            <div class="field">
                ...
                <input id="games-edit-name" type="text" name="name" value="{{ $platform->name }}" class="input" required="required">
            </div>
        </form>
