...
@section('title') {{ _('New publisher') }} @endsection

@section('content')
<section class="template-default template-publishers">
    <div class="container">
        <h1>{{ _('Insert a new publisher') }}</h1>
        ...
        <form action="{{ route('publishers.store')}}" method="post" enctype="multipart/form-data">
            ...
            <div class="field">
                ...
                @enderror
                <label for="publisher-create-name" class="label">{{ _('Name') }}</label>
                <input id="publisher-create-name" type="text" name="name" value="{{ old('name') }}" class="input" required="required">
            </div>
